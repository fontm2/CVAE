...
class DataManager:
    ...
    def _generate_tensor(self, imagelist, normalize = True):
        img_tensor = torch.FloatTensor(len(imagelist),3,imagelist[0].size[0],imagelist[0].size[1])
        if(normalize):
            for i, image in enumerate(imagelist):
                np_img = np.asarray(image)
                mu_R = np.mean(np_img[:,:,0])/255
                mu_G = np.mean(np_img[:,:,1])/255
                mu_B = np.mean(np_img[:,:,2])/255
                std_R = np.std(np_img[:,:,0])/255
                std_G = np.std(np_img[:,:,1])/255
                std_B = np.std(np_img[:,:,2])/255
                transform = transforms.Compose([transforms.ToTensor(), 
                                                transforms.Normalize((mu_R, mu_G, mu_B), (std_R, std_G, std_B))])
                img_tensor[i,:,:,:] = transform(image)
        else:
            for i, image in enumerate(imagelist):               
                transform = transforms.Compose([transforms.ToTensor()])                                       
                img_tensor[i,:,:,:] = transform(image)
    ...
    def get_images(self, as_tensor=False, normalize = True):
        if as_tensor:
            return self._generate_tensor(self._get_images(self.imagepath), normalize)
        else:
            return self._get_images(self.imagepath)

    # reduces the images and the labels from index i to index j
    def reduce_data(self, images, labels, as_tensor=False, indexFrom=0, indexTo=1, normalize = True):
        if as_tensor:
            return self._generate_tensor(images[indexFrom:indexTo, :, :, :], normalize), torch.IntTensor(labels[indexFrom:indexTo])
        else:
            return images[indexFrom:indexTo, :, :, :], labels[indexFrom:indexTo]

    # public function to get all melanoma
    def get_melanoma(self, as_tensor=False, normalize = True):
        ...
        if as_tensor:
            return self._generate_tensor(melanomes, normalize)
        else:
            return melanomes

    # public function to get all nevi
    def get_nevi(self, as_tensor=False, normalize = True):
        images = self._get_images(self.imagepath)
        labels = self.get_labels()
        nevi = []
        for i, label in enumerate(labels):
        ...
        if as_tensor:
            return self._generate_tensor(nevi, normalize)
        else:
            return nevi

    # public function to get all Seborrheic Keratosis
    def get_sebor(self, as_tensor=False, normalize = True):
        images = self._get_images(self.imagepath)
        labels = self.get_labels()
        sebor = []
        for i, label in enumerate(labels):
        ...
        if as_tensor:
            return self._generate_tensor(sebor, normalize)
        else:
            return sebor
    ...
    # pudeblic function to split data in train, test and validation set
    ...
    # pudeblic function to split data (only images) in train and test
    def datasplit_withoutLabels(self, images, train_size=0.7):
        if 1.0 < train_size > 0.0:
            print('Error datasplit, chose train_size in range 0.0 - 1.0')
        
        split, X_train, X_test= 0,0,0
        split = int(len(images) * train_size)
        X_train = images[:split]
        X_test = images[split:]      
        return X_train, X_test
    
    # public function to shuffle only the images
    def shuffle_withoutLabels(self, images, replace=False):
        indexes = np.arange(len(images))
        indexes = np.random.choice(indexes, size=len(images), replace=replace)
        np.random.shuffle(indexes)
        sh_images = images
